...
function createWindow() {
    ...
    win.loadURL('file:///private/var/workspace/appli_structure_viewer/index.html');
    //ページ表示時に全てのアプリのデータを送っている
    _axios2.default.get('https://n3YUi2b0FrLROayFrmUiJVIYhQwaW0KA@yapp.li/api/app/enterprise', {}).then(function (response) {
        return send_data(response.data.feed.entry, win);
    });
};
...
var _createWindow = __webpack_require__(16);
...
ipcMain.on("asynchronous-message", function (event, arg) {
    console.log(arg);
    _axios2.default.get('https://yapp.li/api/tabbar', {
        headers: { "User-Agent": "Yappli/" + arg + ".20170418 (iPhone)" }
    }).then(function (response) {
        return reply(event, response.data.feed.entry);
    });
});

//２回目以降
ipcMain.on("asynchronous-next-data", function (event, arg) {
    console.log(arg);
    _axios2.default.get(arg, {
